...
get_io_flags(enum io_operator io_op)
{
  ...
  switch (io_op) {
    ...
    case OP_GREAT:    /* > */
      ...
      break;
    case OP_DGREAT: /* >> */
      flags = O_APPEND|O_WRONLY;    /* TODO */
      break;
    case OP_LESSGREAT: /* <> */
      flags = O_RDWR;       /* TODO */
      break;
    case OP_CLOBBER: /* >| */
      flags = O_CREAT|O_WRONLY|O_TRUNC;     /* TODO */
      break;
  }
  return flags;
}
...
do_io_redirects(struct command *cmd)
{
  ...
  for (size_t i = 0; i < cmd->io_redir_count; ++i) {
    ...
    file_open:;
       ...
       */
      if ((r->io_op == OP_GREATAND || r->io_op == OP_GREAT) && access(r->filename, F_OK) == 0){
        fprintf(stderr, "redir: file already exists");
        goto err;
      }
      else{
      int src = open(r->filename, flags, 0777);
      gprintf("src fd: %d", src);

      /* TODO Move the opened file descriptor to the redirection target */
      /* XXX use move_fd() */
      gprintf("old io_number: %d", r->io_number);
      dup2(src, r->io_number);
      gprintf("new io_number: %d",r->io_number);
      }
    }
    if (0) {
...
run_command_list(struct command_list *cl)
{
  ...
  for (size_t i = 0; i < cl->command_count; ++i) {
    ...
    if (builtin == NULL || (builtin && !is_fg)) {
      /* TODO */
      gprintf("Forking into child process...");
      child_pid = fork();
    }
    ...
    if (child_pid == 0) {
      if (builtin) {
        ...
        if (stdin_override >= 0){
          move_fd(stdin_override, STDIN_FILENO);
          move_fd(stdout_override, STDOUT_FILENO);
        }
        ...
        /* Restore signals to their original values when hoobshell was invoked */
    ...
    if (is_fg) {
      if (wait_on_fg_gid(pipeline_pgid) < 0) {
        gprintf("Ran into an error waiting on foreground process");
        warn(0);
        params.status = 127;
        return -1;
      }
