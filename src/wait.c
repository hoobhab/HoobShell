...
wait_on_fg_gid(pid_t pgid)
{
  ...
  for (;;) {
    ...
    if (res < 0) {
      ...
      if (errno == ECHILD) {
        ...
        if (WIFEXITED(last_status)) {
          gprintf("last foreground process: %d", WEXITSTATUS(status));
    ...
    if (WIFSTOPPED(status)) {
      ...
      params.status = 128 + WSTOPSIG(status);
      goto out;
    }
...
err:
  ...
  if (isatty(STDIN_FILENO)) {
     ...
     */
    pid_t hoobShellPgid = getpgrp();
    tcsetpgrp(STDIN_FILENO, hoobShellPgid);
  } else {
    switch (errno) {
      case ENOTTY:
        errno = 0;
...
wait_on_bg_jobs()
{
  ...
  for (size_t i = 0; i < job_count; ++i) {
    ...
    for (;;) {
       ...
       */
      int status;
      pid_t pid = waitpid(-pgid, &status, WNOHANG); //"-pgid" fix from discord
      if (pid == 0) {
      ...
      if (WIFSIGNALED(status)) {
        gprintf("Process in bg group stopped");
        fprintf(stderr, "[%jd] STOPPED\n", (intmax_t)jid);
        break;
      }
    }
